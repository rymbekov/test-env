...
class Pullable extends React.Component {
  ...
  componentDidMount() {
    const element = this.props.childrenWrapperClassName
  ...
  render() {
    ...
    return (
      <>
        ...
        <Choose>
          <When condition={childrenWrapperClassName}>
            <div id={this.componentId} className={childrenWrapperClassName}>
              {children}
            </div>
          </When>
...
const Container = styled.div`
  ...
  transition: ${(props) =>
    props.shouldReset ? `height ${props.resetDuration}ms ${props.resetEase}` : 'none'};
  display: flex;
  overflow: hidden;
...
const Spinner = styled.div`
  ...
  transform: ${(props) =>
    props.shouldReset
      ? `translateY(${
          props.pctPulled * (props.spinnerSize + props.spinnerOffset) - props.spinnerSize
        }px) rotate(${props.rotateSpinner && props.shouldSpin ? 90 : 0}deg)`
      : `translateY(${
          props.pctPulled * (props.spinnerSize + props.spinnerOffset) - props.spinnerSize
        }px) rotate(${props.rotateSpinner ? props.pctPulled * 90 : 0}deg)`};
  transition: ${(props) =>
    props.shouldReset
