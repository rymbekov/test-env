/**
 * This file modified by p0k1e
 * link to original file below
 * @link https://github.com/kaisalmen/WWOBJLoader/blob/master/src/loaders/OBJLoader2.js
 */
import _LoaderSupport from './LoaderSupport';
 ...
 */
const OBJLoader2 = function (three) {
  THREE = three;
  const LoaderSupport = _LoaderSupport(three);

  const OBJLOADER2_VERSION = '2.2.1';
  const { LoaderBase } = LoaderSupport;
  const { Validator } = LoaderSupport;
  const { ConsoleLogger } = LoaderSupport;
  ...
  OBJLoader2.prototype.load = function (url, onLoad, onProgress, onError, onMeshAlter, useAsync) {
    ...
    if (!Validator.isValid(onError)) {
      onError = function (event) {
        const output = `Error occurred while downloading "${url}"`;
  ...
  OBJLoader2.prototype.run = function (prepData, workerSupportExternal) {
    ...
    const available = this._checkFiles(prepData.resources);
    ...
    const scope = this;
    const onMaterialsLoaded = function (materials) {
  ...
  OBJLoader2.prototype.parse = function (content) {
    ...
    const scope = this;
    const onMeshLoaded = function (payload) {
      const meshes = scope.builder.processPayload(payload);
      let mesh;
    ...
    const onProgressScoped = function (text, numericalValue) {
  ...
  OBJLoader2.prototype.parseAsync = function (content, onLoad) {
    ...
    const scopedOnMeshLoaded = function (payload) {
      const meshes = scope.builder.processPayload(payload);
      let mesh;
      for (const i in meshes) {
        mesh = meshes[i];
    ...
    const buildCode = function (funcBuildObject, funcBuildSingelton) {
      let workerCode = '';
      workerCode += '/**\n';
    ...
    const materialNames = {};
    const materials = this.builder.getMaterials();
  ...
  var Parser = (function () {
    ...
    Parser.prototype.parse = function (arrayBuffer) {
      ...
      let bufferPointer = 0;
      const slashSpacePattern = new Array(16);
      let slashSpacePatternPointer = 0;
      let code;
      let word = '';
      let i = 0;
      for (; i < length; i++) {
    ...
    Parser.prototype.processLine = function (
      ...
      const countSlashes = function (slashSpacePattern, slashSpacePatternPointer) {
        let slashesCount = 0;
        for (let i = 0; i < slashSpacePatternPointer; i++) {
          slashesCount += slashSpacePattern[i];
        }
        return slashesCount;
      };

      const concatStringBuffer = function (buffer, bufferPointer, slashSpacePattern) {
        let concatBuffer = '';
        if (bufferPointer === 2) {
          concatBuffer = buffer[1];
        } else {
          const bufferLength = bufferPointer - 1;
      ...
      const flushStringBuffer = function (buffer, bufferPointer) {
        for (let i = 0; i < bufferPointer; i++) {
          buffer[i] = '';
        }
      };
    ...
    Parser.prototype.createRawMeshReport = function (rawMesh, inputObjectCount) {
      const report = rawMesh.createReport(inputObjectCount);
      return (
        `Input Object number: ${
          inputObjectCount
        }\n\tObject name: ${
          ...
          report.smoothingGroupCount
        }\n\tMaterial count: ${
          report.mtlCount
        }\n\tReal RawMeshSubGroup count: ${
          report.subGroups}`
      );
    };

    Parser.prototype.processCompletedMesh = function (currentByte) {
      const result = this.rawMesh.finalize();
      if (Validator.isValid(result)) {
        ...
        const progressBytesPercent = currentByte / this.totalBytes;
    ...
    Parser.prototype.finalize = function (currentByte) {
      ...
      if (this.processCompletedMesh(currentByte) && this.logger.isEnabled()) {
        const parserFinalReport =					`${'Overall counts: '
          ...
          this.counts.doubleIndicesCount}`;
    ...
    Parser.prototype.buildMesh = function (result, currentByte) {
      const rawObjectDescriptions = result.subGroups;
      ...
      const createMultiMaterial = rawObjectDescriptions.length > 1;
      let materialIndex = 0;
      ...
      let materialOrg; let material; let materialName; let
        materialNameOrg;
      for (const oodIndex in rawObjectDescriptions) {
        ...
        materialName =					materialNameOrg
					...
					+ (rawObjectDescription.smoothingGroup === 0 ? '_flat' : '');
        materialOrg = this.materials[materialNameOrg];
        material = this.materials[materialName];

        // both original and derived names do not lead to an existing material => need to use a default material
        if (!Validator.isValid(materialOrg) && !Validator.isValid(material)) {
          const defaultMaterialName = haveVertexColors ? 'vertexColorMaterial' : 'defaultMaterial';
          materialOrg = this.materials[defaultMaterialName];
          this.logger.logWarn(
            `object_group "${
              rawObjectDescription.objectName
            }_${
        ...
        if (createMultiMaterial) {
          ...
          materialGroupLength = this.useIndices
            ? rawObjectDescription.indices.length
            : rawObjectDescription.vertices.length / 3;
          materialGroup = {
        ...
        if (this.logger.isDebug()) {
          const materialIndexLine = Validator.isValid(selectedMaterialIndex)
            ? `\n\t\tmaterialIndex: ${selectedMaterialIndex}`
            : '';
          const createdReport =						`Output Object no.: ${
            this.outputObjectCount
          }\n\t\tgroupName: ${
            rawObjectDescription.groupName
          }${materialIndexLine
          }\n\t\tmaterialName: ${
            rawObjectDescription.materialName
          }\n\t\tsmoothingGroup: ${
            rawObjectDescription.smoothingGroup
          }\n\t\tobjectName: ${
            rawObjectDescription.objectName
          }\n\t\t#vertices: ${
            rawObjectDescription.vertices.length / 3
            ...
            rawObjectDescription.colors.length / 3
          }\n\t\t#uvs: ${
            rawObjectDescription.uvs.length / 2
          }\n\t\t#normals: ${
            rawObjectDescription.normals.length / 3}`;
  ...
  var RawMesh = (function () {
    ...
    RawMesh.prototype.pushSmoothingGroup = function (smoothingGroup) {
      ...
      const smoothCheck = this.smoothingGroup.normalized;
      this.smoothingGroup.normalized = this.smoothingGroup.splitMaterials
        ? smoothingGroupInt
        : smoothingGroupInt === 0
          ? 0
          : 1;
    ...
    RawMesh.prototype.verifyIndex = function () {
      const index = `${this.activeMtlName}|${this.smoothingGroup.normalized}`;
      this.subGroupInUse = this.subGroups[index];
      if (!Validator.isValid(this.subGroupInUse)) {
        this.subGroupInUse = new RawMeshSubGroup(
          this.objectName,
          this.groupName,
          this.activeMtlName,
    ...
    RawMesh.prototype.buildFace = function (faceIndexV, faceIndexU, faceIndexN) {
      const sgiu = this.subGroupInUse;
      if (this.disregardNormals) faceIndexN = undefined;
      const scope = this;
      const updateRawObjectDescriptionInUse = function () {
      ...
      if (this.useIndices) {
        ...
        let indicesPointer = sgiu.indexMappings[mappingName];
    ...
    RawMesh.prototype.finalize = function () {
      const rawObjectDescriptionsTemp = [];
      let rawObjectDescription;
      ...
      let indices;
      for (const name in this.subGroups) {
        ...
        if (rawObjectDescription.vertices.length > 0) {
          ...
          if (indices.length > 0 && absoluteIndexMappingsCount > 0) {
            for (const i in indices) indices[i] = indices[i] + absoluteIndexMappingsCount;
          }
  ...
  OBJLoader2.prototype.loadMtl = function (url, content, callbackOnLoad, crossOrigin) {
    const resource = new LoaderSupport.ResourceDescriptor(url, 'MTL');
  ...
  OBJLoader2.prototype._loadMtl = function (resource, callbackOnLoad, crossOrigin) {
    ...
    const materials = [];
    const scope = this;
    const processMaterials = function (materialCreator) {
    ...
    if (!Validator.isValid(resource) || (!Validator.isValid(resource.content) && !Validator.isValid(resource.url))) {
      ...
      if (Validator.isValid(resource.content)) {
        ...
        const onError = function (event) {
          const output = `Error occurred while downloading "${resource.url}"`;
          scope.logger.logError(`${output}: ${event}`);
          throw output;
        };
