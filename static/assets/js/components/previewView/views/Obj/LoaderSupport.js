/* global Parser */
var THREE = null;
const LoaderSupport = function(three) {
	THREE = three;
...
LoaderSupport.WorkerSupport = (function() {
			...
			switch (payload.cmd) {
				...
				case 'complete':
					...
					if (this.runtimeRef.terminateRequested) {
						this.runtimeRef.logger.logInfo(
							'WorkerSupport [' +
				...
				case 'error':
					this.runtimeRef.logger.logError(
						'WorkerSupport [' + this.runtimeRef.runnerImplName + ']: Reported error: ' + payload.msg
					);
					this.runtimeRef.queuedMessage = null;
					this.started = false;
					this.runtimeRef.callbacks.onLoad(payload.msg);
				...
				default:
					this.runtimeRef.logger.logError(
						'WorkerSupport [' + this.runtimeRef.runnerImplName + ']: Received unknown command: ' + payload.cmd
					);
					break;
			}
		};
	...
	WorkerSupport.prototype.validate = function(functionCodeBuilder, libLocations, libPath, runnerImpl) {
		...
		if (Validator.isValid(runnerImpl)) {
			...
			this.logger.logInfo(
				'WorkerSupport: Using DEFAULT "LoaderSupport.WorkerRunnerRefImpl" as Runncer class for worker.'
			);
		}
...
LoaderSupport.WorkerDirector = (function() {
	...
	WorkerDirector.prototype.isRunning = function() {
		...
		return (
			(this.instructionQueue.length > 0 && this.instructionQueuePointer < this.instructionQueue.length) ||
			wsKeys.length > 0
		);
	};
